...
void loop() {
  ...
  if (millis() - timer1 >= 5000 && Temp <= 50 && stateHeater1 == false){ // Устанавливаем время через которое запустится первый нагреватель и температуру
    stateHeater1 = true;
    timer1 = millis();
    digitalWrite(heater_1, HIGH);
    Serial.println("Heater 1 turn on");
  } else if (millis() - timer2 >= 10000 && Temp <=35 && stateHeater2 == false) {
    stateHeater2 = true;
    timer2 = millis();
    digitalWrite(heater_2, HIGH); 
    Serial.println("Heater 2 turn on");
  } else if (Temp >= 45 && stateHeater2 == true){
    stateHeater2 = false;
    digitalWrite(heater_2, LOW);
